...
const copy = {
  ko: {
    ...
    signup: "회원가입", sellerCenter: "상품 등록", bulkOrder: "단체주문", buyerAccount: "구매자 계정", sellerAccount: "판매자 계정",
    ...
    sheetTitle: "공동구매 참여하기", quantity: "구매 수량", total: "선택 인원 가격의 50% 선결제", pricePreview: "예상 인원별 가격 보기", personPrice: "명 예상 가격", priceGuide: "구매자는 슬라이더로 1~100명 가격을 미리 확인할 수 있습니다. 실제 가격은 최종 참여 인원으로 자동 확정됩니다.", paymentNote: "지금 선택 인원 예상가의 50%를 선결제하고, 모집 종료 후 확정가의 나머지 금액을 결제합니다.", join: "공구 참여 예약하기", success: "참여 예약이 완료됐어요!", successBody: "선결제가 예약됐습니다. 모집 종료 후 최종 참여 인원에 따라 가격이 확정됩니다.", close: "확인", orderType:"주문 유형", individualOrder:"개인주문", individualLimit:"최대 3개", groupOrderType:"단체주문", groupLimit:"학교·교회 등 최대 100개", groupOrderName:"학교·교회·단체명", groupContact:"담당자 연락처", resaleWarning:"단체주문 상품은 다른 온라인 판매처에서 재판매할 수 없습니다. 실제 사용 목적의 학교·교회·기관·모임 주문만 가능합니다.", resaleAgree:"온라인 재판매 금지 조건을 확인하고 동의합니다.",
    ...
    productTitle: "새 공구 상품 등록", productBody: "정상가를 입력하면 1~100명 가격표가 자동 생성되고, 100명 가격은 정상가의 50%가 됩니다.", productName: "상품명", sellerName: "판매자명", originalPrice: "정상 판매가", dealPrice: "100명 최종가 (자동 50%)", category: "상품 종류", deadlineLabel: "모집 기간", register: "상품 등록하기", registered: "상품이 등록됐어요!", registeredBody: "등록한 상품이 공구 목록에 바로 표시됩니다.", demo: "샘플에서는 현재 화면에만 등록 내용이 유지됩니다.", required: "모든 정보를 입력해 주세요.", bulkTitle: "단체주문 수기 등록", bulkBody: "사이트 밖에서 실제로 접수한 단체주문을 참여 인원에 합산합니다.", selectProduct: "상품 선택", groupName: "단체명", contact: "담당자 연락처", bulkQuantity: "주문 인원", memo: "메모", bulkRegister: "단체주문 반영하기", bulkSuccess: "단체주문이 반영됐어요!", bulkSuccessBody: "참여 인원에 합산했으며 단체주문 포함으로 구분 표시됩니다.", manualIncluded: "단체주문 포함", noSpace: "남은 모집 인원보다 많이 입력할 수 없습니다.",
  },
  en: {
    brandSub: "A special price made by 100 people", nav: ["Home", "Deals", "How it works"], language: "한국어",
    signup: "Sign up", sellerCenter: "Add product", bulkOrder: "Group order", buyerAccount: "Buyer account", sellerAccount: "Seller account",
    ...
    sheetTitle: "Join this group deal", quantity: "Purchase quantity", total: "50% deposit at selected estimate", pricePreview: "Preview price by group size", personPrice: "-person estimated price", priceGuide: "Move the slider to preview prices from 1 to 100 people. The final price is automatically based on actual participants.", paymentNote: "Pay 50% of the selected estimate now. The remaining amount is settled after the final participant count.", join: "Reserve my spot", success: "Your spot is reserved!", successBody: "Your deposit is reserved. The final price will be based on the participant count when recruitment ends.", close: "Done", orderType:"Order type", individualOrder:"Individual", individualLimit:"Maximum 3", groupOrderType:"Group order", groupLimit:"Up to 100 for schools, churches, etc.", groupOrderName:"School, church, or group name", groupContact:"Coordinator contact", resaleWarning:"Group-order products may not be resold through other online marketplaces. Group orders are for actual use by schools, churches, organizations, and communities only.", resaleAgree:"I agree to the no-online-resale condition.",
    ...
    productTitle: "List a new group deal", productBody: "Enter retail price to generate a 1–100 person price curve ending at 50% off.", productName: "Product name", sellerName: "Seller name", originalPrice: "Retail price", dealPrice: "100-person final price (50%)", category: "Product type", deadlineLabel: "Deal duration", register: "List product", registered: "Product listed!", registeredBody: "Your product now appears in the deal list.", demo: "In this demo, new listings remain only on the current screen.", required: "Please complete all fields.", bulkTitle: "Record a group order", bulkBody: "Add a real group order received outside the site to the participant count.", selectProduct: "Select product", groupName: "Group name", contact: "Contact", bulkQuantity: "Number of participants", memo: "Memo", bulkRegister: "Add group order", bulkSuccess: "Group order added!", bulkSuccessBody: "It is included in participants and clearly labeled as a group order.", manualIncluded: "Includes group order", noSpace: "Quantity cannot exceed the remaining participant capacity.",
  },
};
...
export default function Home() {
  ...
  const addProduct = (e: FormEvent<HTMLFormElement>) => {
    ...
    setProducts(prev => [{ emoji, category, ko: name, en: name, price: retail / 2, original: retail, joined: 0, manualJoined:0, time: [duration, 0], color: "lavender", seller }, ...prev]); setActiveCategory("all");
    setFormError(false); setSellerSuccess(true);
  };
  const addBulkOrder = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault(); const data=new FormData(e.currentTarget); const index=Number(data.get("productIndex")); const amount=Number(data.get("bulkQuantity"));
    if(!data.get("groupName")||!data.get("contact")||!amount){setBulkError(t.required);return;} if(products[index].joined+amount>100){setBulkError(t.noSpace);return;}
    setProducts(prev=>prev.map((p,i)=>i===index?{...p,joined:p.joined+amount,manualJoined:p.manualJoined+amount}:p)); setBulkError(""); setBulkSuccess(true);
  };
  ...
  return <main>
    <header className="topbar">
      ...
      <div className="headerActions">
        ...
        {!account ? <button className="accountButton" onClick={() => setAccountOpen(true)}>{t.signup}</button> : account.role === "seller" ? <><button className="bulkHeaderButton" onClick={()=>{setBulkSuccess(false);setBulkOpen(true);}}>＋ {t.bulkOrder}</button><button className="accountButton sellerButton" onClick={() => {setSellerSuccess(false); setSellerOpen(true);}}>＋ {t.sellerCenter}</button></> : <span className="accountPill">● {t.buyerAccount}</span>}
      </div>
    </header>
    ...
    {dealOpen && <div className="overlay" onMouseDown={()=>setDealOpen(false)}><section className="sheet orderSheet" onMouseDown={e=>e.stopPropagation()} role="dialog" aria-modal="true"><button className="sheetClose" onClick={()=>setDealOpen(false)}>×</button>{!dealSuccess ? <><span className="sheetEmoji">{product.emoji}</span><span className="sectionKicker">{product.seller}</span><h2>{t.sheetTitle}</h2><p className="sheetProduct">{lang === "ko" ? product.ko : product.en}</p><div className="sheetProgress"><div><b>{product.joined}{lang === "ko" ? "명" : ""}</b><span>/ 100</span></div><div className="progress"><span style={{width:`${product.joined}%`}} /></div><small>{100-product.joined}{t.left}</small></div><div className="orderType"><span>{t.orderType}</span><div><button className={orderType==="individual"?"active":""} onClick={()=>{setOrderType("individual");setQuantity(Math.min(3,quantity));setResaleAgreed(false);}}><b>👤 {t.individualOrder}</b><small>{t.individualLimit}</small></button><button className={orderType==="group"?"active":""} onClick={()=>setOrderType("group")}><b>🏫 {t.groupOrderType}</b><small>{t.groupLimit}</small></button></div></div>{orderType==="group"&&<div className="groupOrderFields"><div><input aria-label={t.groupOrderName} placeholder={t.groupOrderName} onChange={e=>setGroupNameReady(Boolean(e.target.value))}/><input aria-label={t.groupContact} placeholder={t.groupContact} onChange={e=>setGroupContactReady(Boolean(e.target.value))}/></div><p>⚠ {t.resaleWarning}</p><label><input type="checkbox" checked={resaleAgreed} onChange={e=>setResaleAgreed(e.target.checked)}/><span>{t.resaleAgree}</span></label></div>}<div className="priceSimulator"><div><span>{t.pricePreview}</span><strong>{previewCount}{t.personPrice} · {won(previewPrice,lang)}</strong></div><input aria-label={t.pricePreview} type="range" min="1" max="100" value={previewCount} onChange={e=>setPreviewCount(Number(e.target.value))}/><div className="rangeLabels"><span>1{lang==="ko"?"명":""} · {won(priceAtCount(product,1),lang)}</span><span>100{lang==="ko"?"명":""} · {won(product.price,lang)}</span></div><small>{t.priceGuide}</small></div><div className="quantity"><span>{t.quantity} <small>({orderType==="group"?t.groupLimit:t.individualLimit})</small></span><div><button onClick={()=>setQuantity(Math.max(1,quantity-1))} disabled={quantity===1}>−</button><b>{quantity}</b><button onClick={()=>setQuantity(Math.min(orderType==="group"?100:3,quantity+1))} disabled={quantity===(orderType==="group"?100:3)}>＋</button></div></div><div className="total"><span>{t.total}</span><div><del>{lang === "ko" ? `예상 총액 ${won(previewPrice*quantity,lang)}` : `Estimated total ${won(previewPrice*quantity,lang)}`}</del><strong>{won(deposit,lang)}</strong><b>50%</b></div></div><p className="notice">ⓘ {t.paymentNote}</p><button className="primary joinButton" disabled={orderType==="group"&&(!resaleAgreed||!groupNameReady||!groupContactReady)} onClick={()=>setDealSuccess(true)}>{t.join}</button></> : <div className="success"><div>✓</div><h2>{t.success}</h2><p>{t.successBody}</p><button className="primary" onClick={()=>setDealOpen(false)}>{t.close}</button></div>}</section></div>}
    ...
    {bulkOpen && <div className="overlay" onMouseDown={()=>setBulkOpen(false)}><section className="sheet sellerSheet" onMouseDown={e=>e.stopPropagation()} role="dialog" aria-modal="true"><button className="sheetClose" onClick={()=>setBulkOpen(false)}>×</button>{!bulkSuccess?<><span className="sectionKicker">GROUP ORDER</span><h2>{t.bulkTitle}</h2><p className="sheetProduct">{t.bulkBody}</p><form onSubmit={addBulkOrder} className="form sellerForm"><label>{t.selectProduct}<select name="productIndex">{products.map((p,i)=><option key={`${p.ko}-${i}`} value={i}>{p.emoji} {lang==="ko"?p.ko:p.en} ({p.joined}/100)</option>)}</select></label><div className="formRow"><label>{t.groupName}<input name="groupName" placeholder={lang==="ko"?"예: 행복아파트 입주자회":"e.g. Community group"}/></label><label>{t.contact}<input name="contact" placeholder="010-0000-0000"/></label></div><div className="formRow"><label>{t.bulkQuantity}<input name="bulkQuantity" type="number" min="1" max="100" placeholder="10"/></label><label>{t.memo}<input name="memo" placeholder={lang==="ko"?"담당자 또는 배송 메모":"Contact or delivery note"}/></label></div>{bulkError&&<p className="formError">{bulkError}</p>}<p className="notice">ⓘ {lang==="ko"?"실제로 접수한 단체주문만 입력하세요. 단체주문 인원은 별도로 표시됩니다.":"Enter only real group orders. Manual participants are labeled separately."}</p><button className="primary joinButton">{t.bulkRegister}</button></form></>:<div className="success"><div>✓</div><h2>{t.bulkSuccess}</h2><p>{t.bulkSuccessBody}</p><button className="primary" onClick={()=>setBulkOpen(false)}>{t.close}</button></div>}</section></div>}
